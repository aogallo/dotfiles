...
- **FR-003**: A line consisting only of whitespace MUST become an empty line; it MUST NOT be removed, and paragraph separation MUST be preserved. This is a guarantee of the whitespace-only fallback, which trims the line and stops there. When the main formatter runs instead, its own blank-line normalization applies: a line of only whitespace between two blank lines is collapsed, exactly as any other blank line would be, and that is the formatter's own behavior rather than a cleanup effect.
...
- **FR-006**: The cleanup MUST NOT alter content inside the line: leading indentation and tabs MUST survive, and only whitespace at the end of the line may change. This is a guarantee of the whitespace-only fallback, which touches the line end and nothing else. When the main formatter runs instead, its own reformatting applies to code as well as to spacing, and this requirement does not restrain it.
