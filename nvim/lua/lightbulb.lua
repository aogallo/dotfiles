...
local function get_lsp_diagnostics(bufnr, line)
    -- textDocument/codeAction expects LSP-shaped diagnostics, not vim.Diagnostic.
    return vim.lsp.diagnostic.from(vim.diagnostic.get(bufnr, { lnum = line }))
end
...
local function render(bufnr, client)
    ...
    local diagnostics = get_lsp_diagnostics(bufnr, line)
